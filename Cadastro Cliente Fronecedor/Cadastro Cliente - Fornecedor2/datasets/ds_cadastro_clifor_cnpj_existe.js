function createDataset(fields, constraints, sortFields) {
	...
	for (var i = 0; i < constraints.length; i++) {
		...
		if (constraints[i].fieldName == "CGCCFO") {
			descricao = constraints[i].initialValue;
		}
	}

	var campos = new Array("NOMEFANTASIA", "CGCCFO");
	...
	var c4 = DatasetFactory.createConstraint("CGCCFO", descricao,
	...
	if (datasetRM == null || datasetRM == undefined) {
		...
		for (var i = 0; i < datasetRM.rowsCount; i++) {
			...
			dataset.addRow(new Array(NOMEFANTASIA, CGCCFO));
		}
