...
function getQuery(FILIAL){
	
	return "" +
	"SELECT" +
	" ( CAST(GFILIAL.CODFILIAL AS varchar) + ' - ' + GFILIAL.NOMEFANTASIA)  AS FILIAL," +
	" GFILIAL.CODFILIAL" +
	
	" FROM GFILIAL (NOLOCK)" +
	
	" WHERE CODCOLIGADA IN (1,3)" +
	" AND ( CAST(GFILIAL.CODFILIAL AS varchar) + ' - ' + GFILIAL.NOMEFANTASIA) LIKE '%"+FILIAL+"%' " +
