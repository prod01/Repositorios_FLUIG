function createDataset(fields, constraints, sortFields) {
	...
	for (var i = 0; i < constraints.length; i++) {
		...
		if (constraints[i].fieldName == "CENTROCUSTO") {
			descricao = constraints[i].initialValue;
		}
	}

	var campos = new Array("CENTROCUSTO");
	...
	var c4 = DatasetFactory.createConstraint("CENTROCUSTO", descricao,
	...
	if (datasetRM == null || datasetRM == undefined) {
		...
		for (var i = 0; i < datasetRM.rowsCount; i++) {
			var CENTROCUSTO = datasetRM.getValue(i, "CENTROCUSTO");
			dataset.addRow(new Array(CENTROCUSTO));
		}
